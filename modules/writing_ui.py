...
def count_stats(text):
    """Calculates text statistics: words, characters, and paragraphs."""
...
def writing_ui(language, debug_mode):
    """
    Renders the Writing Mode page, with debugging, translation, and robust data handling.
    """
    ...
    palavras_ativas = sorted(db_df[db_df['ativo'] == True]['palavra'].tolist())
    ...
    if st.session_state.get('word_for_text_area') != palavra_atual:
        ...
        if writing_log:
            ...
            if not log_df.empty and 'palavra' in log_df.columns:
